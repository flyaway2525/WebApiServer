...
set "ENV_EXAMPLE=%ROOT_DIR%\api\.env.example"
set "ENV_FILE=%ROOT_DIR%\api\.env"

if "%~1"=="" goto install
if /i "%~1"=="install" goto install
if /i "%~1"=="dev" goto dev
if /i "%~1"=="dev:api" goto dev_api
if /i "%~1"=="dev:web" goto dev_web
...
echo Run "setup.bat dev" to start the API and Web clients with the local Node.js runtime.
exit /b 0
...
echo.
echo Commands:
echo   setup.bat            Install local Node.js, copy api\.env, and run npm install
echo   setup.bat install    Same as no arguments
echo   setup.bat dev        Start both the API and Web clients with local Node.js
echo   setup.bat dev:api    Start only the API with local Node.js
echo   setup.bat dev:web    Start only the Web client with local Node.js
echo   setup.bat build      Build the API and Web packages with local Node.js
...
copy "%ENV_EXAMPLE%" "%ENV_FILE%" >nul || exit /b 1

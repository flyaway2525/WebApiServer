...
function ensureSameSpace(member: SpaceMemberRecord | null, spaceId: number, fieldName: string) {
  if (member && member.spaceId !== spaceId) {
    throw new Error(`${fieldName} must belong to the selected space`);
  }
}

export async function validateReferencedMembers(
  spaceId: number,
  input: {
...
export function assertTransactionRequestPermission(
  sessionMember: SpaceMemberRecord,
  space: SpaceRecord,

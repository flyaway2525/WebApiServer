...
async function insertTransactionEntry(spaceId, transaction) {
    const client = getClient();
    await client.execute({
        sql: `
...
async function getAggregatedSpaceById(spaceId) {
    const client = getClient();
    const result = await client.execute({
        sql: `
      SELECT
...
async function getSpaceMemberById(memberId) {
    const client = getClient();
    const result = await client.execute({
        sql: `
      SELECT
...
export async function createSpaceTransaction(spaceId, input) {
    const client = getClient();
